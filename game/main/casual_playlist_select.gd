...
# Casual's own playlist picker -- near-duplicate of ranked_playlist_select.gd
# (no ELO/tier display, casual has none) since the two screens' actual
# content diverges enough (accent color, target scene, party restriction
# messaging is identical but the underlying queue differs -- see
# casual_queue.gd vs ranked_queue.gd) that sharing a base class would cost
# more than the small duplication saves.
...
func _ready() -> void:
	...
	UIStyle.apply_layout_override(title_label, "casual_playlist_select.title")
	UIStyle.apply_layout_override(subtitle_label, "casual_playlist_select.subtitle")
	UIStyle.apply_layout_override(back_button, "casual_playlist_select.back_button")
...
func _build_card(playlist_id: String) -> Button:
	...
	UIStyle.apply_layout_override(btn, "casual_playlist_select.card_%s" % playlist_id)
	UIStyle.apply_layout_override(name_label, "casual_playlist_select.card_%s_name" % playlist_id)
	UIStyle.apply_layout_override(sub_label, "casual_playlist_select.card_%s_sub" % playlist_id)
	return btn

func _on_playlist_pressed(playlist_id: String) -> void:
	GameSettings.selected_casual_playlist = playlist_id
	get_tree().change_scene_to_file("res://main/casual_queue.tscn")
