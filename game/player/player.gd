...
# Physics layer 1 ("world") only -- players/NPCs never physically collide
# with each other at all (collision_mask is world-only too, see
# player.tscn), so this exists mainly for clarity at each raycast call site
# rather than to actually exclude anything this body's own mask wouldn't
# already exclude.
